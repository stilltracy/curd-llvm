...
#ifdef ENABLE_CURD
 allocateReadWriteSets(grid, block, EXPANDED_SIZE(dim_block)*sizeof(unsigned int));
#endif
        scan_inter1_kernel<<<grid, block, EXPANDED_SIZE(dim_block)*sizeof(unsigned int)>>>(inter_d, stride);
#ifdef ENABLE_CURD
freeReadWriteSets(grid, block, EXPANDED_SIZE(dim_block)*sizeof(unsigned int));
#endif
