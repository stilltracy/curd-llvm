...
#ifdef ENABLE_CURD
 allocateReadWriteSets( DimPhiMagGrid, DimPhiMagBlock );
#endif
  ComputePhiMag_GPU <<< DimPhiMagGrid, DimPhiMagBlock >>> 
    (phiR_d, phiI_d, phiMag_d, numK);
#ifdef ENABLE_CURD
freeReadWriteSets( DimPhiMagGrid, DimPhiMagBlock );
#endif
}
...
#ifdef ENABLE_CURD
 allocateReadWriteSets( DimQGrid, DimQBlock );
#endif
    ComputeQ_GPU <<< DimQGrid, DimQBlock >>>
      (numK, QGridBase, x_d, y_d, z_d, Qr_d, Qi_d);
#ifdef ENABLE_CURD
freeReadWriteSets( DimQGrid, DimQBlock );
#endif
  }
}
