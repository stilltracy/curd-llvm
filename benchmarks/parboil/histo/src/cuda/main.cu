...
#ifdef ENABLE_CURD
 allocateReadWriteSets(dim3(PRESCAN_BLOCKS_X),dim3(PRESCAN_THREADS));
#endif
    histo_prescan_kernel<<<dim3(PRESCAN_BLOCKS_X),dim3(PRESCAN_THREADS)>>>((unsigned int*)input, img_height*img_width, ranges);
#ifdef ENABLE_CURD
freeReadWriteSets(dim3(PRESCAN_BLOCKS_X),dim3(PRESCAN_THREADS));
#endif
...
#ifdef ENABLE_CURD
 allocateReadWriteSets(dim3((img_height + UNROLL-1)/UNROLL), dim3((img_width+1)/2));
#endif
...
#ifdef ENABLE_CURD
freeReadWriteSets(dim3((img_height + UNROLL-1)/UNROLL), dim3((img_width+1)/2));
#endif
...
#ifdef ENABLE_CURD
 allocateReadWriteSets(dim3(BLOCK_X, ranges_h[1]-ranges_h[0]+1), dim3(THREADS));
#endif
...
#ifdef ENABLE_CURD
freeReadWriteSets(dim3(BLOCK_X, ranges_h[1]-ranges_h[0]+1), dim3(THREADS));
#endif
    
    pb_SwitchToSubTimer(&timers, finals, pb_TimerID_KERNEL);
    
#ifdef ENABLE_CURD
 allocateReadWriteSets(dim3(BLOCK_X*3), dim3(512));
#endif
...
#ifdef ENABLE_CURD
freeReadWriteSets(dim3(BLOCK_X*3), dim3(512));
#endif
  }
  pb_SwitchToTimer(&timers, pb_TimerID_IO);
